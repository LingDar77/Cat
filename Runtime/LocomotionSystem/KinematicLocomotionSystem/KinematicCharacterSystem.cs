    ...
    public class KinematicCharacterSystem : MonoBehaviour
    {
        ...
        public static void PreSimulationInterpolationUpdate(float deltaTime)
        {
            ...
            for (int i = 0; i < CharacterMotors.Count; i++)
            {
                ...
                if (motor.CharacterController == null) continue;

                var config = motor.CharacterController.GetConfig();

                if (!config.ShouldDoInterpolate) continue;
...
#pragma warning disable 0162
            ...
            for (int i = 0; i < characterMotorsCount; i++)
            {
                ...
                if (!config.ShouldDoSimulation) continue;
...
#pragma warning restore 0162
        ...
        public static void CustomInterpolationUpdate()
        {
            ...
            for (int i = 0; i < CharacterMotors.Count; i++)
            {
                ...
                if (motor.CharacterController == null) continue;
                var config = motor.CharacterController.GetConfig();
                if (!config.ShouldDoCustomInterpolate) continue;

                motor.Transform.SetPositionAndRotation(Vector3.Lerp(motor.InitialSimulationPosition, motor.TransientPosition, interpolationFactor), config.ShouldDoCustomRotationInterpolate ? Quaternion.Slerp(motor.InitialTickRotation, motor.TransientRotation, interpolationFactor) : motor.InitialTickRotation);
            }
